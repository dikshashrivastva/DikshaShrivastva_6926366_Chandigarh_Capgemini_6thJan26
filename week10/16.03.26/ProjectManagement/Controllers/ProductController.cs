    ...
    public class ProductController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var products = new List<Product>
            {
                ...
                new Product { Id = 4, Name = "Desk Lamp",         Category = "Furniture",   Price = 599.00m,  Stock = 20  },
        ...
        {
            _logger.LogInformation("[ProductController] SimulateError action started.");

            if (!_env.IsDevelopment())
            {
                // Avoid simulating an error in non-development environments
                return RedirectToAction(nameof(Index));
            }

            // Instead of throwing an exception (which is poor practice for control flow),
            // return the friendly Error view used by the CustomExceptionFilter. This
            // preserves the demonstration intent without actually throwing.
            var errorModel = new ErrorViewModel
            {
                Message = "Simulated error (development only).",
                ExceptionType = nameof(InvalidOperationException),
                Detail = "This is a test exception thrown from ProductController.SimulateError to demonstrate the CustomExceptionFilter."
            };
